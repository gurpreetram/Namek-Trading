...
export default {
  async fetch(request) {
    ...
    try {
      ...
      if (path === '/industry-strength') return jsonResponse(await getIndustryStrength());
      ...
      if (path === '/industry-alignment-test-baseline') return await handleIndustryAlignmentBaseline(url);
      if (path === '/industry-alignment-test-aligned') return await handleIndustryAlignmentAligned(url);
      ...
      if (path === '/daily-picks-industry-boost-test') return await handleDailyPicksIndustryBoostTest(url);
      if (path === '/daily-picks-full-universe-page') return await handleDailyPicksFullUniversePage(url);
...
const PAGE_SIZE = 15; // reduced from 22 — the industry strength engine (Phase 2 v2) now costs up to 19 calls (1 per industry) instead of the old 9-sector engine's 9, so the true worst case is 1(symbols)+8(marketDirection)+19(industryStrength)+pageSize, and this keeps a real ~7-request safety margin under the 50-subrequest cap rather than cutting it close
...
// ============================================================
// INDUSTRY STRENGTH ENGINE v2 — replaces the 9-sector/65-stock map above for
// LIVE scoring (the old map + SECTOR_INDEX_MAP above stay in place untouched,
// still used only by the historical backtest sector-alignment path below,
// which needs 60-day index candles the proxy-stock approach here doesn't fetch).
//
// Real problem found: the old map only covered ~65 hand-picked large-caps
// across 9 broad sectors, so most of the scan universe got zero sector
// signal, and stocks trending on real market data (e.g. Groww's own
// sector-trending page, which ranks ~60 granular industries) never got
// picked up here. This map was built from NSE's OWN current official
// classification — fetched live from nseindia.com's Nifty 100 + Midcap 150 +
// Smallcap 100 constituent lists, not a stale third-party mirror (an earlier
// candidate dataset was checked and rejected for containing since-delisted/
// merged symbols like MINDTREE, LTI, INFRATEL, JETAIRWAYS). 350 stocks across
// NSE's 19 official industry classes — a large majority of what actually
// shows up in an NSE 500-scoped intraday scan.
const STOCK_INDUSTRY_MAP = {
  'APOLLOTYRE.NS': 'Automobile and Auto Components', 'ARE&M.NS': 'Automobile and Auto Components', 'ATHERENERG.NS': 'Automobile and Auto Components', 'BAJAJ-AUTO.NS': 'Automobile and Auto Components', 'BALKRISIND.NS': 'Automobile and Auto Components', 'BHARATFORG.NS': 'Automobile and Auto Components', 'BOSCHLTD.NS': 'Automobile and Auto Components', 'EICHERMOT.NS': 'Automobile and Auto Components', 'ENDURANCE.NS': 'Automobile and Auto Components', 'EXIDEIND.NS': 'Automobile and Auto Components', 'FORCEMOT.NS': 'Automobile and Auto Components', 'HEROMOTOCO.NS': 'Automobile and Auto Components', 'HYUNDAI.NS': 'Automobile and Auto Components', 'JBMA.NS': 'Automobile and Auto Components', 'M&M.NS': 'Automobile and Auto Components', 'MARUTI.NS': 'Automobile and Auto Components', 'MOTHERSON.NS': 'Automobile and Auto Components', 'MRF.NS': 'Automobile and Auto Components', 'OLAELEC.NS': 'Automobile and Auto Components', 'SCHAEFFLER.NS': 'Automobile and Auto Components', 'SONACOMS.NS': 'Automobile and Auto Components', 'TENNIND.NS': 'Automobile and Auto Components', 'TIINDIA.NS': 'Automobile and Auto Components', 'TMPV.NS': 'Automobile and Auto Components', 'TVSMOTOR.NS': 'Automobile and Auto Components', 'UNOMINDA.NS': 'Automobile and Auto Components',
  'ABB.NS': 'Capital Goods', 'AIAENG.NS': 'Capital Goods', 'APARINDS.NS': 'Capital Goods', 'APLAPOLLO.NS': 'Capital Goods', 'ASHOKLEY.NS': 'Capital Goods', 'ASTRAL.NS': 'Capital Goods', 'BDL.NS': 'Capital Goods', 'BEL.NS': 'Capital Goods', 'BEML.NS': 'Capital Goods', 'BHEL.NS': 'Capital Goods', 'CGPOWER.NS': 'Capital Goods', 'COCHINSHIP.NS': 'Capital Goods', 'CUMMINSIND.NS': 'Capital Goods', 'DATAPATTNS.NS': 'Capital Goods', 'ENRIN.NS': 'Capital Goods', 'ESCORTS.NS': 'Capital Goods', 'GPIL.NS': 'Capital Goods', 'GRSE.NS': 'Capital Goods', 'GVT&D.NS': 'Capital Goods', 'HAL.NS': 'Capital Goods', 'HBLENGINE.NS': 'Capital Goods', 'HONAUT.NS': 'Capital Goods', 'INOXWIND.NS': 'Capital Goods', 'JYOTICNC.NS': 'Capital Goods', 'KAYNES.NS': 'Capital Goods', 'KEI.NS': 'Capital Goods', 'MAZDOCK.NS': 'Capital Goods', 'POLYCAB.NS': 'Capital Goods', 'POWERINDIA.NS': 'Capital Goods', 'PREMIERENE.NS': 'Capital Goods', 'SIEMENS.NS': 'Capital Goods', 'SUPREMEIND.NS': 'Capital Goods', 'SUZLON.NS': 'Capital Goods', 'THERMAX.NS': 'Capital Goods', 'TMCV.NS': 'Capital Goods', 'TRITURBINE.NS': 'Capital Goods', 'WAAREEENER.NS': 'Capital Goods', 'WELCORP.NS': 'Capital Goods',
  'AARTIIND.NS': 'Chemicals', 'CHAMBLFERT.NS': 'Chemicals', 'COROMANDEL.NS': 'Chemicals', 'DEEPAKFERT.NS': 'Chemicals', 'FLUOROCHEM.NS': 'Chemicals', 'HSCL.NS': 'Chemicals', 'LINDEINDIA.NS': 'Chemicals', 'NAVINFLUOR.NS': 'Chemicals', 'PIDILITIND.NS': 'Chemicals', 'PIIND.NS': 'Chemicals', 'SOLARINDS.NS': 'Chemicals', 'SRF.NS': 'Chemicals', 'SWANCORP.NS': 'Chemicals', 'TATACHEM.NS': 'Chemicals', 'UPL.NS': 'Chemicals',
  'AFCONS.NS': 'Construction', 'IRCON.NS': 'Construction', 'KEC.NS': 'Construction', 'LT.NS': 'Construction', 'NBCC.NS': 'Construction', 'RVNL.NS': 'Construction',
  'ACC.NS': 'Construction Materials', 'AMBUJACEM.NS': 'Construction Materials', 'DALBHARAT.NS': 'Construction Materials', 'GRASIM.NS': 'Construction Materials', 'JKCEMENT.NS': 'Construction Materials', 'JSWCEMENT.NS': 'Construction Materials', 'RAMCOCEM.NS': 'Construction Materials', 'SHREECEM.NS': 'Construction Materials', 'ULTRACEMCO.NS': 'Construction Materials',
  'AMBER.NS': 'Consumer Durables', 'ASIANPAINT.NS': 'Consumer Durables', 'BERGEPAINT.NS': 'Consumer Durables', 'BLUESTARCO.NS': 'Consumer Durables', 'CROMPTON.NS': 'Consumer Durables', 'DIXON.NS': 'Consumer Durables', 'HAVELLS.NS': 'Consumer Durables', 'KALYANKJIL.NS': 'Consumer Durables', 'LGEINDIA.NS': 'Consumer Durables', 'PGEL.NS': 'Consumer Durables', 'TITAN.NS': 'Consumer Durables', 'VOLTAS.NS': 'Consumer Durables', 'WHIRLPOOL.NS': 'Consumer Durables',
  'BLS.NS': 'Consumer Services', 'DEVYANI.NS': 'Consumer Services', 'DMART.NS': 'Consumer Services', 'ETERNAL.NS': 'Consumer Services', 'FIRSTCRY.NS': 'Consumer Services', 'INDHOTEL.NS': 'Consumer Services', 'IRCTC.NS': 'Consumer Services', 'ITCHOTELS.NS': 'Consumer Services', 'JUBLFOOD.NS': 'Consumer Services', 'LENSKART.NS': 'Consumer Services', 'MEESHO.NS': 'Consumer Services', 'NAUKRI.NS': 'Consumer Services', 'NYKAA.NS': 'Consumer Services', 'PWL.NS': 'Consumer Services', 'SWIGGY.NS': 'Consumer Services', 'TRENT.NS': 'Consumer Services', 'URBANCO.NS': 'Consumer Services', 'VMM.NS': 'Consumer Services',
  '3MINDIA.NS': 'Diversified', 'GODREJIND.NS': 'Diversified',
  'AWL.NS': 'Fast Moving Consumer Goods', 'BRITANNIA.NS': 'Fast Moving Consumer Goods', 'COLPAL.NS': 'Fast Moving Consumer Goods', 'DABUR.NS': 'Fast Moving Consumer Goods', 'GODFRYPHLP.NS': 'Fast Moving Consumer Goods', 'GODREJCP.NS': 'Fast Moving Consumer Goods', 'HINDUNILVR.NS': 'Fast Moving Consumer Goods', 'ITC.NS': 'Fast Moving Consumer Goods', 'MARICO.NS': 'Fast Moving Consumer Goods', 'NESTLEIND.NS': 'Fast Moving Consumer Goods', 'PATANJALI.NS': 'Fast Moving Consumer Goods', 'RADICO.NS': 'Fast Moving Consumer Goods', 'TATACONSUM.NS': 'Fast Moving Consumer Goods', 'UBL.NS': 'Fast Moving Consumer Goods', 'UNITDSPR.NS': 'Fast Moving Consumer Goods', 'VBL.NS': 'Fast Moving Consumer Goods',
  '360ONE.NS': 'Financial Services', 'ABCAPITAL.NS': 'Financial Services', 'AIIL.NS': 'Financial Services', 'ANANDRATHI.NS': 'Financial Services', 'ANGELONE.NS': 'Financial Services', 'APTUS.NS': 'Financial Services', 'AUBANK.NS': 'Financial Services', 'AXISBANK.NS': 'Financial Services', 'BAJAJFINSV.NS': 'Financial Services', 'BAJAJHFL.NS': 'Financial Services', 'BAJAJHLDNG.NS': 'Financial Services', 'BAJFINANCE.NS': 'Financial Services', 'BANDHANBNK.NS': 'Financial Services', 'BANKBARODA.NS': 'Financial Services', 'BANKINDIA.NS': 'Financial Services', 'BSE.NS': 'Financial Services', 'CAMS.NS': 'Financial Services', 'CANBK.NS': 'Financial Services', 'CDSL.NS': 'Financial Services', 'CGCL.NS': 'Financial Services', 'CHOLAFIN.NS': 'Financial Services', 'CHOLAHLDNG.NS': 'Financial Services', 'CREDITACC.NS': 'Financial Services', 'CRISIL.NS': 'Financial Services', 'CUB.NS': 'Financial Services', 'FEDERALBNK.NS': 'Financial Services', 'FIVESTAR.NS': 'Financial Services', 'GICRE.NS': 'Financial Services', 'GROWW.NS': 'Financial Services', 'HDBFS.NS': 'Financial Services', 'HDFCAMC.NS': 'Financial Services', 'HDFCBANK.NS': 'Financial Services', 'HDFCLIFE.NS': 'Financial Services', 'HUDCO.NS': 'Financial Services', 'ICICIAMC.NS': 'Financial Services', 'ICICIBANK.NS': 'Financial Services', 'ICICIGI.NS': 'Financial Services', 'ICICIPRULI.NS': 'Financial Services', 'IDBI.NS': 'Financial Services', 'IDFCFIRSTB.NS': 'Financial Services', 'IFCI.NS': 'Financial Services', 'IIFL.NS': 'Financial Services', 'INDIANB.NS': 'Financial Services', 'INDUSINDBK.NS': 'Financial Services', 'IREDA.NS': 'Financial Services', 'IRFC.NS': 'Financial Services', 'JIOFIN.NS': 'Financial Services', 'JMFINANCIL.NS': 'Financial Services', 'KARURVYSYA.NS': 'Financial Services', 'KFINTECH.NS': 'Financial Services', 'KOTAKBANK.NS': 'Financial Services', 'LICHSGFIN.NS': 'Financial Services', 'LICI.NS': 'Financial Services', 'LTF.NS': 'Financial Services', 'M&MFIN.NS': 'Financial Services', 'MAHABANK.NS': 'Financial Services', 'MANAPPURAM.NS': 'Financial Services', 'MCX.NS': 'Financial Services', 'MFSL.NS': 'Financial Services', 'MOTILALOFS.NS': 'Financial Services', 'MUTHOOTFIN.NS': 'Financial Services', 'NAM-INDIA.NS': 'Financial Services', 'NIACL.NS': 'Financial Services', 'NUVAMA.NS': 'Financial Services', 'PAYTM.NS': 'Financial Services', 'PFC.NS': 'Financial Services', 'PINELABS.NS': 'Financial Services', 'PIRAMALFIN.NS': 'Financial Services', 'PNB.NS': 'Financial Services', 'PNBHOUSING.NS': 'Financial Services', 'POLICYBZR.NS': 'Financial Services', 'POONAWALLA.NS': 'Financial Services', 'RBLBANK.NS': 'Financial Services', 'RECLTD.NS': 'Financial Services', 'SBICARD.NS': 'Financial Services', 'SBILIFE.NS': 'Financial Services', 'SBIN.NS': 'Financial Services', 'SHRIRAMFIN.NS': 'Financial Services', 'STARHEALTH.NS': 'Financial Services', 'SUNDARMFIN.NS': 'Financial Services', 'TATACAP.NS': 'Financial Services', 'TATAINVEST.NS': 'Financial Services', 'UNIONBANK.NS': 'Financial Services', 'YESBANK.NS': 'Financial Services',
  ...
  'AFFLE.NS': 'Information Technology', 'COFORGE.NS': 'Information Technology', 'HCLTECH.NS': 'Information Technology', 'HEXT.NS': 'Information Technology', 'IKS.NS': 'Information Technology', 'INFY.NS': 'Information Technology', 'KPITTECH.NS': 'Information Technology', 'LTM.NS': 'Information Technology', 'LTTS.NS': 'Information Technology', 'MPHASIS.NS': 'Information Technology', 'NETWEB.NS': 'Information Technology', 'OFSS.NS': 'Information Technology', 'PERSISTENT.NS': 'Information Technology', 'SAGILITY.NS': 'Information Technology', 'TATAELXSI.NS': 'Information Technology', 'TATATECH.NS': 'Information Technology', 'TCS.NS': 'Information Technology', 'TECHM.NS': 'Information Technology', 'WIPRO.NS': 'Information Technology', 'ZENSARTECH.NS': 'Information Technology',
  'ADANIENT.NS': 'Metals & Mining', 'GMDCLTD.NS': 'Metals & Mining', 'HINDALCO.NS': 'Metals & Mining', 'HINDCOPPER.NS': 'Metals & Mining', 'HINDZINC.NS': 'Metals & Mining', 'JINDALSTEL.NS': 'Metals & Mining', 'JSL.NS': 'Metals & Mining', 'JSWSTEEL.NS': 'Metals & Mining', 'LLOYDSME.NS': 'Metals & Mining', 'NATIONALUM.NS': 'Metals & Mining', 'NMDC.NS': 'Metals & Mining', 'SAIL.NS': 'Metals & Mining', 'SARDAEN.NS': 'Metals & Mining', 'TATASTEEL.NS': 'Metals & Mining', 'VEDL.NS': 'Metals & Mining',
  'AEGISLOG.NS': 'Oil Gas & Consumable Fuels', 'ATGL.NS': 'Oil Gas & Consumable Fuels', 'BPCL.NS': 'Oil Gas & Consumable Fuels', 'CASTROLIND.NS': 'Oil Gas & Consumable Fuels', 'COALINDIA.NS': 'Oil Gas & Consumable Fuels', 'GAIL.NS': 'Oil Gas & Consumable Fuels', 'HINDPETRO.NS': 'Oil Gas & Consumable Fuels', 'IGL.NS': 'Oil Gas & Consumable Fuels', 'IOC.NS': 'Oil Gas & Consumable Fuels', 'MRPL.NS': 'Oil Gas & Consumable Fuels', 'OIL.NS': 'Oil Gas & Consumable Fuels', 'ONGC.NS': 'Oil Gas & Consumable Fuels', 'PETRONET.NS': 'Oil Gas & Consumable Fuels', 'RELIANCE.NS': 'Oil Gas & Consumable Fuels',
  'ADANIENSOL.NS': 'Power', 'ADANIGREEN.NS': 'Power', 'ADANIPOWER.NS': 'Power', 'CESC.NS': 'Power', 'JSWENERGY.NS': 'Power', 'NHPC.NS': 'Power', 'NLCINDIA.NS': 'Power', 'NTPC.NS': 'Power', 'NTPCGREEN.NS': 'Power', 'POWERGRID.NS': 'Power', 'RPOWER.NS': 'Power', 'SJVN.NS': 'Power', 'TATAPOWER.NS': 'Power', 'TORNTPOWER.NS': 'Power',
  'ABREL.NS': 'Realty', 'ANANTRAJ.NS': 'Realty', 'BRIGADE.NS': 'Realty', 'DLF.NS': 'Realty', 'GODREJPROP.NS': 'Realty', 'LODHA.NS': 'Realty', 'OBEROIRLTY.NS': 'Realty', 'PHOENIXLTD.NS': 'Realty', 'PRESTIGE.NS': 'Realty', 'SIGNATURE.NS': 'Realty',
  'ADANIPORTS.NS': 'Services', 'CONCOR.NS': 'Services', 'DELHIVERY.NS': 'Services', 'FSL.NS': 'Services', 'GESHIP.NS': 'Services', 'GMRAIRPORT.NS': 'Services', 'INDIGO.NS': 'Services', 'JSWINFRA.NS': 'Services', 'REDINGTON.NS': 'Services',
  'BHARTIARTL.NS': 'Telecommunication', 'BHARTIHEXA.NS': 'Telecommunication', 'IDEA.NS': 'Telecommunication', 'INDUSTOWER.NS': 'Telecommunication', 'ITI.NS': 'Telecommunication', 'TATACOMM.NS': 'Telecommunication',
  'KPRMILL.NS': 'Textiles', 'PAGEIND.NS': 'Textiles',
};

// One liquid, representative proxy stock per industry (not 2-3) — a real bug
// was found here: with all industries at up to 3 proxies each, this fired 55
// concurrent fetchQuote calls in one Promise.all, blowing well past
// Cloudflare's 50-subrequest-per-invocation cap on whichever scan page
// happened to trigger the 3-minute cache refresh (confirmed live: the two
// industries missing from a real /industry-strength response were
// Telecommunication and Textiles — alphabetically LAST, exactly where a
// silent cutoff at the subrequest cap would land). One proxy per industry
// (19 calls total) restores the same real safety margin the original
// 9-sector design had with its 1-quote-per-sector approach.
const INDUSTRY_PROXY_STOCKS = {
  'Automobile and Auto Components': ['MARUTI.NS'],
  'Capital Goods': ['SIEMENS.NS'],
  'Chemicals': ['PIDILITIND.NS'],
  'Construction': ['LT.NS'],
  'Construction Materials': ['ULTRACEMCO.NS'],
  'Consumer Durables': ['ASIANPAINT.NS'],
  'Consumer Services': ['DMART.NS'],
  'Diversified': ['3MINDIA.NS'],
  'Fast Moving Consumer Goods': ['HINDUNILVR.NS'],
  'Financial Services': ['HDFCBANK.NS'],
  'Healthcare': ['SUNPHARMA.NS'],
  'Information Technology': ['TCS.NS'],
  'Metals & Mining': ['TATASTEEL.NS'],
  'Oil Gas & Consumable Fuels': ['RELIANCE.NS'],
  'Power': ['NTPC.NS'],
  'Realty': ['DLF.NS'],
...
let industryStrengthCache = { data: null, fetchedAt: 0 };
const INDUSTRY_STRENGTH_CACHE_MS = 3 * 60 * 1000;

// Same rank-based scoring pattern as getSectorStrength() below, but ranks all
// 19 real industries using their live proxy-stock quotes (averaged) instead
// of a dedicated index ticker — genuinely mirrors how Groww ranks industries
// by today's % change, just computed from data this app already fetches.
async function getIndustryStrength() {
  const now = Date.now();
  if (industryStrengthCache.data && (now - industryStrengthCache.fetchedAt) < INDUSTRY_STRENGTH_CACHE_MS) {
    return industryStrengthCache.data;
  }

  const industryNames = Object.keys(INDUSTRY_PROXY_STOCKS);
  const perIndustryQuotes = await Promise.all(
    industryNames.map(name => Promise.all(INDUSTRY_PROXY_STOCKS[name].map(sym => fetchQuote(sym).catch(() => null))))
  );

  const ranked = industryNames
    .map((name, i) => {
      const valid = perIndustryQuotes[i].filter(q => q !== null);
      if (valid.length === 0) return null;
      const avgChangePct = valid.reduce((sum, q) => sum + q.changePct, 0) / valid.length;
      return { name, changePct: avgChangePct, sampleSize: valid.length };
    })
    .filter(s => s !== null)
  ...
  industryStrengthCache = { data: result, fetchedAt: now };
  return result;
}
...
// Historical counterpart to getIndustryStrength()'s live proxy-stock approach
// — fetches 60-day 5-min candles for each industry's single proxy stock (19
// fetches, one per industry, same shape/cost as fetchHistoricalSectorCandles
// above), so the NEW 19-industry map can finally be backtested honestly
// instead of only ever running live. Built specifically to answer whether
// the broader industry map actually improves win rate/EV, not just whether
// it looks reasonable live.
// Accepts an explicit list of industry names to fetch — callers should pass
// only the industries actually needed by the symbols being tested, not all
// 19 unconditionally. REAL FIX found after a live Error 1102 (CPU limit):
// the original unconditional version fetched and parsed 60-day candles for
// all 19 industries every time, even when a given backtest batch (e.g. the
// 12-stock default sample) only ever needed a handful — for that sample,
// just 5 of the 19 industries were actually used, so 14 full candle-series
// fetches were pure waste. Defaults to all 19 only if no list is given.
async function fetchHistoricalIndustryCandles(onlyIndustries = null) {
  const industryNames = onlyIndustries || Object.keys(INDUSTRY_PROXY_STOCKS);
  const results = await Promise.all(
    industryNames.map(name => fetchCandles(INDUSTRY_PROXY_STOCKS[name][0], '5m', '60d').catch(() => null))
  );
  const byName = {};
  industryNames.forEach((name, i) => { byName[name] = results[i] || []; });
  return byName;
}
...
// Point-in-time counterpart to live getIndustryStrength() — reuses the exact
// same getHistoricalSectorChangePct binary-search lookup (generic, works on
// any candle array) to build a real rank/strengthScore snapshot for a set of
// industries AT a given historical timestamp, in the same {byName: {name:
// {name, changePct, rank, strengthScore}}} shape verifyStock's sectorInfo
// lookup already expects live. Built specifically to test whether the
// existing (but never isolated) sector-strength scoring boost — up to 10 of
// 100 points — actually helps Daily Top Picks' win rate, as opposed to the
// hard alignment GATE tested earlier (which didn't replicate). This is a
// SOFT boost test: stocks aren't rejected, just scored differently.
function getHistoricalIndustryRankingAt(historicalIndustryCandles, industryNames, targetTimestamp, lookbackMinutes = 30) {
  const ranked = industryNames
    .map(name => {
      const changePct = getHistoricalSectorChangePct(historicalIndustryCandles[name], targetTimestamp, lookbackMinutes);
      return changePct == null ? null : { name, changePct };
    })
    .filter(s => s !== null)
    .sort((a, b) => b.changePct - a.changePct);
...
async function handleScanMarket(url) {
  ...
  const sectorStrength = await getIndustryStrength(); // industry-level (350 stocks/19 industries), replacing the old 9-sector index engine for live scoring
  ...
  const topSelections = smoothedValid
    ...
    .filter(card => Math.abs(card.sessionChangePct ?? 0) >= 1.5) // MOMENTUM FILTER, applied per explicit request to the main Top Opportunities list — a real gap found: this list previously had no minimum price-move requirement at all, only score and risk:reward
...
// ============================================================
async function runBacktest(symbol, sectorStrength, requireSectorAlignment = false, historicalSectorCandles = null, requireDailyPicksFilter = false, requireAlternativeConfidence75 = false, requireIndustryAlignment = false, historicalIndustryCandles = null, useIndustryBoost = false, industryNamesForBoost = null) {
  ...
  const stockIndustry = STOCK_INDUSTRY_MAP[symbol] || null; // NEW 19-industry map, for the industry-alignment A/B test
  ...
  for (let i = MIN_HISTORY; i < candles.length; i++) {
    ...
    // SOFT INDUSTRY BOOST (opt-in, isolated for honest testing): unlike the
    // hard alignment GATE tested earlier (which rejects trades and didn't
    // replicate), this only affects SCORING — feeds a real point-in-time
    // industry ranking into verifyStock exactly as live scanning already
    // does, so the existing (but never isolated) up-to-10-point sector
    // strength category actually gets exercised in this backtest instead of
    // always defaulting to neutral via sectorStrength=null.
    const effectiveSectorStrength = (useIndustryBoost && industryNamesForBoost && historicalIndustryCandles)
      ? getHistoricalIndustryRankingAt(historicalIndustryCandles, industryNamesForBoost, candles[i].t, 30)
      : sectorStrength;
    const result = await verifyStock(symbol, effectiveSectorStrength, null, null, null, historySoFar);
    ...
    // INDUSTRY ALIGNMENT (opt-in, tested honestly, same mechanism as sector
    // alignment above): confirms a stock's move against its OWN industry
    // from the new 350-stock/19-industry map, using getHistoricalSectorChangePct
    // reused as-is since it's a generic candle-array lookback function.
    if (requireIndustryAlignment && stockIndustry && historicalIndustryCandles) {
      const industryChangePct = getHistoricalSectorChangePct(historicalIndustryCandles[stockIndustry], candles[i].t, 30);
      if (industryChangePct == null) continue; // not enough historical industry data at this point — honestly skip rather than assume alignment
      const isLong = result.verdict === 'BUY';
      const industryConfirms = isLong ? industryChangePct > 0 : industryChangePct < 0;
      if (!industryConfirms) continue; // the stock's own industry was NOT moving the same direction — reject, per the filter being tested
    }
...
// ---------- ROUTE: /daily-picks-full-universe-page ----------
// Paginated Daily Top Picks backtest across the FULL ~2200-stock NSE
// universe (not just the 24-stock diverse sample) — same pagination trick
// the live scanner already uses to stay under Cloudflare's per-invocation
// CPU/subrequest limits, applied to backtesting for the first time. One
// request tests one small page; call repeatedly (page=0,1,2...) until
// isLastPage is true, then sum wins/losses across all pages for the true
// aggregate. Manually pasting ~150-200 individual URLs isn't practical —
// this exists so a script can do it instead (see accompanying PowerShell
// script). Uses the EXACT Daily Top Picks bar (requireDailyPicksFilter=
// true), no sector/industry boost or alignment — that idea was tested four
// times already and showed zero measurable effect, so it's excluded here to
// keep full-universe runs as cheap and reliable as possible.
const DAILY_PICKS_FULL_UNIVERSE_PAGE_SIZE = 15;
async function handleDailyPicksFullUniversePage(url) {
  const page = parseInt(url.searchParams.get('page') || '0', 10);
  const symbolsResp = await handleFullSymbols(url);
  const symbolsData = await symbolsResp.json();
  if (symbolsData.error) return jsonResponse({ error: symbolsData.error }, 502);
  const allSymbols = symbolsData.symbols || [];

  const start = page * DAILY_PICKS_FULL_UNIVERSE_PAGE_SIZE;
  const pageSymbols = allSymbols.slice(start, start + DAILY_PICKS_FULL_UNIVERSE_PAGE_SIZE);
  const isLastPage = (start + DAILY_PICKS_FULL_UNIVERSE_PAGE_SIZE) >= allSymbols.length;
  const totalPages = Math.ceil(allSymbols.length / DAILY_PICKS_FULL_UNIVERSE_PAGE_SIZE);

  if (pageSymbols.length === 0) {
    return jsonResponse({ page, isLastPage: true, totalSymbols: allSymbols.length, totalPages, pageResults: { sampleSize: 0, totalTrades: 0, wins: 0, losses: 0 }, note: 'No symbols on this page — backtest complete or page number out of range.' });
  }

  try {
    const results = await Promise.all(
      pageSymbols.map(sym => runBacktest(sym, null, false, null, true).catch(err => ({ symbol: sym, error: err.message })))
    );
    const summary = summarizeSectorTestResults(results);
    return jsonResponse({
      page, isLastPage, totalSymbols: allSymbols.length, totalPages,
      symbolsThisPage: pageSymbols,
      pageResults: summary,
      timestamp: Date.now(),
    });
  } catch (err) {
    return jsonResponse({ error: `Full-universe backtest page ${page} failed: ${err.message}`, page }, 500);
  }
}

// ---------- ROUTE: /daily-picks-industry-boost-test ----------
// Tests a DIFFERENT idea than the alignment gate above, per explicit
// request: not "reject trades whose industry disagrees," but "let a
// genuinely trending industry contribute to the score of Daily Top Picks
// candidates, the same way score already works for everything else." Uses
// the EXACT Daily Top Picks qualification bar (requireDailyPicksFilter=true,
// same as /daily-picks-filter-test-filtered) PLUS the soft industry-strength
// scoring boost, so it can be compared directly against that existing
// filtered-baseline result — same symbols, same qualification bar, only
// difference is whether industry strength is scored (boost) or neutral
// (existing baseline route, which passes sectorStrength=null throughout).
async function handleDailyPicksIndustryBoostTest(url) {
  const customList = url.searchParams.get('symbols');
  // Defaults to 8 stocks (not the full 24) — this route's ranking snapshot
  // runs on EVERY evaluated candle (needed before verifyStock can score),
  // not just qualifying BUY/SELL signals like the alignment gate did, so its
  // per-eval cost is structurally higher. Learned from two live Error 1102s
  // already this session on related routes — starting smaller and letting
  // ?symbols= expand it, rather than guessing a bigger default is safe.
  const symbols = customList ? customList.split(',').map(s => s.trim()) : BACKTEST_DIVERSE_SAMPLE.filter(s => STOCK_INDUSTRY_MAP[s]).slice(0, 8);
  try {
    const neededIndustries = [...new Set(symbols.map(s => STOCK_INDUSTRY_MAP[s]).filter(Boolean))];
    const historicalIndustryCandles = await fetchHistoricalIndustryCandles(neededIndustries);
    const boostedResults = await Promise.all(
      symbols.map(sym => runBacktest(sym, null, false, null, true, false, false, null, true, neededIndustries).catch(err => ({ symbol: sym, error: err.message })))
    );
    return jsonResponse({
      symbolsTested: symbols,
      industriesFetched: neededIndustries,
      dailyTopPicksWithIndustryBoost: summarizeSectorTestResults(boostedResults),
      note: 'Tests the SOFT industry-strength scoring boost (not a hard reject gate) on top of the exact Daily Top Picks qualification bar. Compare expectedValuePerTradeR against /daily-picks-filter-test-filtered run with the SAME symbols — that route is the true baseline here since it already uses sectorStrength=null (neutral scoring) throughout.',
      timestamp: Date.now(),
    });
  } catch (err) {
    return jsonResponse({ error: `Daily Top Picks industry boost test failed: ${err.message}` }, 500);
  }
}

// ---------- ROUTE: /industry-alignment-test-baseline ----------
// Same split-into-two A/B pattern as /sector-alignment-test-*, but tests the
// NEW 350-stock/19-industry map built and fixed this session, instead of the
// old 65-stock/9-sector one. This is what actually answers "did today's
// industry-remap work" for backtest purposes, not the live app alone.
async function handleIndustryAlignmentBaseline(url) {
  const customList = url.searchParams.get('symbols');
  const symbols = customList ? customList.split(',').map(s => s.trim()) : BACKTEST_DIVERSE_SAMPLE.filter(s => STOCK_INDUSTRY_MAP[s]);
  try {
    const baselineResults = await Promise.all(
      symbols.map(sym => runBacktest(sym, null, false, null, false, false, false, null).catch(err => ({ symbol: sym, error: err.message })))
    );
    return jsonResponse({
      symbolsTested: symbols,
      baseline: summarizeSectorTestResults(baselineResults),
      note: 'This is HALF of the A/B comparison — the baseline (no industry alignment required), matching existing live behavior. Run /industry-alignment-test-aligned separately with the SAME symbols to get the comparison half, then compare expectedValuePerTradeR between the two.',
      timestamp: Date.now(),
    });
  } catch (err) {
    return jsonResponse({ error: `Industry alignment baseline test failed: ${err.message}` }, 500);
  }
}

// ---------- ROUTE: /industry-alignment-test-aligned ----------
// The other half: requires the stock's own (new-map) industry to be moving
// the same direction, using real historical proxy-stock candles.
async function handleIndustryAlignmentAligned(url) {
  const customList = url.searchParams.get('symbols');
  const symbols = customList ? customList.split(',').map(s => s.trim()) : BACKTEST_DIVERSE_SAMPLE.filter(s => STOCK_INDUSTRY_MAP[s]);
  try {
    // REAL FIX (not the sample-size cut tried first, which didn't help):
    // only fetch historical candles for the industries these SPECIFIC
    // symbols actually belong to, not all 19 unconditionally — for the
    // default 24-stock sample that's typically ~9 industries, not 19.
    const neededIndustries = [...new Set(symbols.map(s => STOCK_INDUSTRY_MAP[s]).filter(Boolean))];
    const historicalIndustryCandles = await fetchHistoricalIndustryCandles(neededIndustries);
    const alignedResults = await Promise.all(
      symbols.map(sym => runBacktest(sym, null, false, null, false, false, true, historicalIndustryCandles).catch(err => ({ symbol: sym, error: err.message })))
    );
    return jsonResponse({
      symbolsTested: symbols,
      industriesFetched: neededIndustries,
      withIndustryAlignment: summarizeSectorTestResults(alignedResults),
      note: 'This is HALF of the A/B comparison — industry confirmation REQUIRED, using the NEW 350-stock/19-industry map and real historical proxy-stock candles (timestamp-aligned, not approximated). Compare expectedValuePerTradeR against /industry-alignment-test-baseline run with the SAME symbols. Only fetches historical candles for the industries actually used by the tested symbols (see industriesFetched), not all 19 unconditionally — the earlier version did that and hit Error 1102 as a result.',
      timestamp: Date.now(),
    });
  } catch (err) {
    return jsonResponse({ error: `Industry alignment test failed: ${err.message}` }, 500);
  }
}
...
// REAL FIX for intermittent 503s found via live diagnosis: neither fetch()
// call below had any timeout. Certain obscure/thinly-traded NSE symbols
// (SME-listed, delisted-but-still-EQ-series, etc.) appear to make Yahoo
// Finance hang rather than error, so the fetch just never resolved — which
// blocked the WHOLE batch request until Cloudflare itself killed the
// invocation, regardless of batch size (confirmed: identical failures at
// batch sizes 20, 10, AND 5, always around the same obscure symbols — a
// sizing/CPU issue would not behave that way). An 8-second hard timeout via
// AbortController means one bad symbol can now only ever cost 8 seconds
// and return null (handled gracefully everywhere already), instead of
...
async function fetchQuote(symbol) {
  ...
  let res;
  try {
    res = await fetchWithTimeout(yahooUrl, { headers: ua() });
  } catch (err) {
    return null; // timeout or network failure on this ONE symbol — handled gracefully, doesn't block the batch
  }
  if (!res.ok) return null;
  const data = await res.json();
...
async function verifyStock(symbol, sectorStrength = null, patternResult = null, timeframeResult = null, newsResult = null, preloadedCandles = null, niftyChangePct = null) {
  ...
  const sectorName = STOCK_INDUSTRY_MAP[symbol] || null; // live scoring uses the new 350-stock/19-industry map — STOCK_SECTOR_MAP above stays reserved for the historical backtest sector-alignment path only
