...
export function useMarketScan() {
  ...
  // DAILY MOVERS (Full Screener) — built per explicit request: Full
  // Screener should have its OWN locked, validated 2-3 gainers + 2-3
  // losers for the day, the SAME lock-for-the-day + exit-monitoring
  // mechanism as Daily Top Picks, but using gainer/loser-specific
  // criteria (real price momentum) rather than the BUY/SELL scoring
  // engine — kept genuinely SEPARATE from Daily Top Picks rather than
  // reused, since the two answer different questions (best scored
  // setups vs. strongest genuine real-momentum movers).
  const [dailyMovers, setDailyMovers] = useState(restored?.dailyMovers || []);
  const dailyMoversLockedRef = useRef(restored?.dailyMovers?.length > 0 ? restored.dailyMovers : null);
  ...
  const resetDailyMovers = useCallback(() => {
    dailyMoversLockedRef.current = null;
    setDailyMovers([]);
  }, []);
  ...
  const runFullScan = useCallback(async (universeMode = 'full') => {
    ...
    try {
      ...
      // DAILY MOVERS (Full Screener) — locks 2 genuine gainers + 2 genuine
      // losers for the day, mirroring Daily Top Picks' exact mechanism but
      // using gainer/loser-specific criteria: real (non-fake) momentum and
      // a meaningful >=1.5% price move, same threshold as the spec's
      // Momentum Filter applied elsewhere in this app.
      const allResultsSorted = [...resultsBySymbol.values()];
      const qualifiesAsMover = r => r.momentumQuality === 'REAL (built gradually)' && Math.abs(r.sessionChangePct ?? 0) >= 1.5;

      if (!dailyMoversLockedRef.current) {
        const eligibleMovers = allResultsSorted.filter(qualifiesAsMover);
        const topGainers = eligibleMovers.filter(r => r.sessionChangePct > 0).sort((a, b) => b.sessionChangePct - a.sessionChangePct).slice(0, 2);
        const topLosers = eligibleMovers.filter(r => r.sessionChangePct < 0).sort((a, b) => a.sessionChangePct - b.sessionChangePct).slice(0, 2);
        const movers = [...topGainers, ...topLosers].map(r => ({
          ...r,
          isGainer: r.sessionChangePct > 0,
        ...
        if (movers.length > 0) {
          dailyMoversLockedRef.current = movers;
          setDailyMovers(movers);
        }
      } else {
        // EXIT MONITORING for Daily Movers — re-check each held mover's
        // CURRENT real-time data against the same qualification it locked
        // in on: did momentum stop being real, or did the price move
        // shrink back below the meaningful threshold (the move that
        // justified picking it has genuinely faded)?
        const updatedMovers = dailyMoversLockedRef.current.map(held => {
          const current = resultsBySymbol.get(held.symbol);
          if (!current) return held; // not in this scan — don't flag on absence alone
          if (held.exitSignal) {
            const priceHistory = [...(held.priceHistory || [held.price]), current.price].slice(-12);
            return { ...held, priceHistory };
          }

          const reasons = [];
          if (current.momentumQuality !== 'REAL (built gradually)') {
            reasons.push(`Momentum is no longer confirmed as real (now: ${current.momentumQuality})`);
          }
          if (Math.abs(current.sessionChangePct ?? 0) < 1.5) {
            reasons.push(`Price move has faded back below the 1.5% meaningful-move threshold (now ${current.sessionChangePct}%)`);
          }
          if (held.isGainer && current.sessionChangePct < 0) {
            reasons.push(`Direction reversed — this was a gainer, now showing a negative move (${current.sessionChangePct}%)`);
          }
          if (!held.isGainer && current.sessionChangePct > 0) {
            reasons.push(`Direction reversed — this was a loser, now showing a positive move (${current.sessionChangePct}%)`);
          }

          const priceHistory = [...(held.priceHistory || [held.price]), current.price].slice(-12);
          if (reasons.length > 0) {
            return { ...held, price: current.price, sessionChangePct: current.sessionChangePct, priceHistory, lifecycleStatus: 'EXIT', removedAt: Date.now(), exitSignal: { reasons, flaggedAt: Date.now() } };
          }
          return { ...held, price: current.price, sessionChangePct: current.sessionChangePct, priceHistory };
        });
        dailyMoversLockedRef.current = updatedMovers;
        setDailyMovers(updatedMovers);
      }
    } catch (err) {
  ...
  useEffect(() => {
    ...
    saveScanState({ allResults, topSelections, dailyTopPicks, dailyMovers, universe, lastScanCompletedAt });
  }, [allResults, topSelections, dailyTopPicks, dailyMovers, universe, lastScanCompletedAt]);

  const MIN_STOCK_PRICE = 100; // ₹100 floor per explicit request — Full Screener should not surface penny/low-priced stocks

  const actionable = allResults
    .filter(r => ACTIONABLE_VERDICTS.includes(r.verdict))
    .filter(r => Math.abs(r.sessionChangePct ?? 0) >= 1.5) // MOMENTUM FILTER, applied per explicit request to ALL Full Screener tabs (BUY/SELL/Strong Move/Watching) with no exceptions — a real gap found: these tabs previously had no minimum price-move requirement at all
    .filter(r => (r.price ?? 0) >= MIN_STOCK_PRICE)
